-- ============================================
-- VIBEBEAUTY SUPER APP - COMPLETE SUPABASE MIGRATION
-- Idempotent: Tekrar çalıştırılabilir (IF NOT EXISTS / DROP IF EXISTS)
-- ============================================
...
CREATE POLICY "payment_intents_read" ON payment_intents
  FOR SELECT USING (
    auth.uid() = user_id 
    OR auth.uid() IN (SELECT user_id FROM profiles WHERE id = business_id)
  );
...
CREATE POLICY "customer_notes_read" ON customer_notes
  FOR SELECT USING (
    auth.uid() IN (SELECT user_id FROM profiles WHERE id = business_id) 
...
-- MIGRATION COMPLETE
-- ============================================
-- Bu SQL dosyası idempotent'tir (tekrar çalıştırılabilir)
-- Tüm tablolar, policy'ler ve index'ler güvenli bir şekilde oluşturulur
-- ============================================
